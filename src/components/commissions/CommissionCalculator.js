...
const CommissionCalculator = ({
  ...
  const totalExpenses = expenses.reduce(
    (sum, expense) => sum + parseNumber(expense.cost),
    0
  );
  const grossProfit =
    parseNumber(soldPrice) - parseNumber(purchasePrice) - totalExpenses;
  ...
  const adjustedGrossProfit =
    grossProfit - totalFlatSellerCommissions - totalFlatBuyerCommissions;
    ...
    commissions
      ...
      .reduce(
        (sum, commission) =>
          sum +
          adjustedGrossProfit *
            (parseNumber(commission.amount.replace("%", "")) / 100),
        0
      );

  const totalPercentageSellerCommissions =
    calculatePercentageCommissions(sellerCommissions);
  const totalPercentageBuyerCommissions =
    calculatePercentageCommissions(buyerCommissions);
